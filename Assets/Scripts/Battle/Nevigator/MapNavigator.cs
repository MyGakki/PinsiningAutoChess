...
public class MapNavigator
{
    ...
    {
        //初始化一定数量的导航数据
        navigationDataPool = new List<NavigationData>(99);
        for (int i = 0; i < 99; ++i)
        {
        ...
        int stepLimit = -1)
    {
        ...
        while (retry <= tryTimes && !catched)
        {
            ...
            while (checkTimes > 0)
            {
                ...
                {
                    //获取该路通向的下一个item
                    GridUnit sibling = battleMap.GetGridByDir(next_0.thisGrid.GridPosition.x,
                        next_0.thisGrid.GridPosition.y, curDir);
                    if (sibling == null)
                    {
                        //没路
                        ++curDir;
...
//        {
//            sb.Append(gridUnit.ToString() + " ");
//        }
//        UtilityHelper.Log(sb);
//        
        //有步数限制
        if (catched
            && path != null
